...
class Tworkflowpak extends TObjetStd
{
	/**
	 * Draft status
	 */
	...
	public function __construct()
	{
		...
		$this->set_table(MAIN_DB_PREFIX.'workflowpak');
...
//		$this->setChild('TworkflowpakChild','fk_workflowpak');
		...
		$this->generic->element = 'workflowpak';
...
//		global $user;
	...
	private function getNextNumero()
	{
		...
		$numero = get_next_value($db, $mask, 'workflowpak', 'ref');
...
//		global $user;
	...
	public function getNomUrl($withpicto=0, $get_params='')
	{
        ...
        $result='';
        $label = '<u>' . $langs->trans("Showworkflowpak") . '</u>';
        if (! empty($this->ref)) $label.= '<br><b>'.$langs->trans('Ref').':</b> '.$this->ref;
        
        $linkclose = '" title="'.dol_escape_htmltag($label, 1).'" class="classfortooltip">';
        $link = '<a href="'.dol_buildpath('/workflowpak/card.php', 1).'?id='.$this->getId(). $get_params .$linkclose;
	...
	public static function LibStatut($status, $mode)
	{
		...
		if ($status==self::STATUS_DRAFT) { $statustrans='statut0'; $keytrans='workflowpakStatusDraft'; $shortkeytrans='Draft'; }
		if ($status==self::STATUS_VALIDATED) { $statustrans='statut1'; $keytrans='workflowpakStatusValidated'; $shortkeytrans='Validate'; }
		if ($status==self::STATUS_REFUSED) { $statustrans='statut5'; $keytrans='workflowpakStatusRefused'; $shortkeytrans='Refused'; }
		if ($status==self::STATUS_ACCEPTED) { $statustrans='statut6'; $keytrans='workflowpakStatusAccepted'; $shortkeytrans='Accepted'; }
 ...
 */
class Workflowpak extends Tworkflowpak
{
	private $PDOdb;
...
/*
class TworkflowpakChild extends TObjetStd
{
	public function __construct()
	{
		$this->set_table(MAIN_DB_PREFIX.'workflowpak_child');
		
		$this->add_champs('fk_workflowpak', array('type' => 'integer', 'index' => true));
